...
from models.syngin import SynGINClassifier
from models.semgsat import SemGSATNClassifier
...
class Instructor:
    def __init__(self, opt):
        ...
        if 'bert' in opt.model_name:
            ...
            trainset = ABSASSINData(opt.dataset_file['train'], tokenizer, opt=opt)
            testset = ABSASSINData(opt.dataset_file['test'], tokenizer, opt=opt)
        else:    
                ...
                if global_step % self.opt.log_step == 0:
                    ...
                    if test_acc > max_test_acc:
                        ...
                        if test_acc > max_test_acc_overall:
                            if not os.path.exists('./SSIN/state_dict'):
                                os.mkdir('./SSIN/state_dict')
                            model_path = './SSIN/state_dict/{}_{}_acc_{:.4f}_f1_{:.4f}'.format(self.opt.model_name, self.opt.dataset, test_acc, f1)
...
def main():
    model_classes = {
        'atae_lstm': ATAE_LSTM,
        'ian': IAN,
        'syngin': SynGINClassifier,
        'semgsat': SemGSATClassifier,
        'SSIN': SSINClassifier,
    }
    
    dataset_files = {
        'restaurant': {
            'train': './SSIN/dataset/Restaurants_corenlp/train.json',
            'test': './SSIN/dataset/Restaurants_corenlp/test.json',
        },
        'laptop': {
            'train': './SSIN/dataset/Laptops_corenlp/train.json',
            'test': './DSSIN/dataset/Laptops_corenlp/test.json'
        },
        'twitter': {
            'train': './SSIN/dataset/Tweets_corenlp/train.json',
            'test': './SSIN/dataset/Tweets_corenlp/test.json',
        }
    }
    ...
    input_colses = {
        ...
        'ian': ['text', 'aspect'],
        'syngin': ['text', 'aspect', 'pos', 'head', 'deprel', 'post', 'mask', 'length', 'adj'],
        'semgsat': ['text', 'aspect', 'pos', 'head', 'deprel', 'post', 'mask', 'length'],
        'SSIN': ['text', 'aspect', 'pos', 'head', 'deprel', 'post', 'mask', 'length', 'adj'],
    }
    ...
    parser.add_argument('--model_name', default='SSIN', type=str, help=', '.join(model_classes.keys()))
    ...
    parser.add_argument('--vocab_dir', type=str, default='./SSIN/dataset/Laptops_corenlp')
    ...
    if not os.path.exists('./SSIN/log'):
        os.makedirs('./SSIN/log', mode=0o777)
    log_file = '{}-{}-{}.log'.format(opt.model_name, opt.dataset, strftime("%Y-%m-%d_%H:%M:%S", localtime()))
    logger.addHandler(logging.FileHandler("%s/%s" % ('./SSIN/log', log_file)))
